...
exports.loginEmail = catchAsync(async (req, res, next) => {
	...
	if (user.googleId || user.facebookId) {
		return next(new AppError(ERRORS.INVALID.WRONG_CREDENTIAL_ERROR_EMAIL, STATUS_CODE.UNAUTHORIZED));
	}
...
exports.loginPhone = catchAsync(async (req, res, next) => {
	...
	if (user.googleId || user.facebookId) {
		return next(new AppError(ERRORS.INVALID.WRONG_CREDENTIAL_ERROR_PHONE, STATUS_CODE.UNAUTHORIZED));
	}
	//user existance and password is correct
	if (!user || !(await user.correctPassword(password, user.password))) {
		return next(new AppError(ERRORS.INVALID.WRONG_CREDENTIAL_ERROR_PHONE, STATUS_CODE.UNAUTHORIZED));
